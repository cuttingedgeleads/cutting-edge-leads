...
export default async function PurchasedLeadsPage({
  ...
  const totalPages = Math.max(1, Math.ceil(purchasedCount / PAGE_SIZE));
  const currentPage = Math.min(requestedPage, totalPages);
  const skip = (currentPage - 1) * PAGE_SIZE;

  const purchasedLeadRows = await prisma.$queryRaw<{ id: string }[]>(Prisma.sql`
    SELECT l.id
    FROM "Lead" l
    JOIN "LeadUnlockRequest" u
      ON u."leadId" = l.id AND u.status = 'APPROVED'
    GROUP BY l.id
    ORDER BY MAX(u."approvedAt") DESC
    OFFSET ${skip} LIMIT ${PAGE_SIZE}
  `);

  const leadIds = purchasedLeadRows.map((row) => row.id);
